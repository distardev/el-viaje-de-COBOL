...
int runGame(){
    ...
    int enemiesKilledForPowerUp = 0;
    int enemiesToSpawnPowerUp = 5 + rand() % 6;
    ...
    while (window.isOpen()) {
        ...
        // 5) Colisión bala enemiga - jugador
        if (powerUp.isActive() && player.getBounds().intersects(powerUp.getBounds())) {
            int newLevel = player.getFireLevel() + 1;
            player.setFireLevel(newLevel);
            powerUp.deactivate();
        ...
        for (auto it = enemies.begin(); it != enemies.end(); ) {
            if (!it->isAlive()) {
                ...
                // Solo soltar power-up si ya hay menos power-ups que (nivel - 1)
                if (powerUpsSpawned < (level - 1)) {
                    sf::FloatRect spawnBounds(minX, minY, maxX - minX, maxY - minY);
                    powerUp.spawnRandomly(spawnBounds, resources.powerUpTexture);
                    powerUpsSpawned++;
                    droppedPowerUp = true;
                }

                // Si no soltó power-up, tiene 10% de soltar moneda
                if (!droppedPowerUp && (rand() % 100 < 10)) {
